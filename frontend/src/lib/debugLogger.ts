/**
 * Android PWA など console が取りづらい環境向けの汎用診断ロガー。使い方は
 * [@docs/debug_logger.md](../../../docs/debug_logger.md) を参照。
 *
 * IndexedDB に ring buffer (最大 500 件) で書き、`DebugLogPanel` の Copy Logs で
 * 一括取得する。SW からも同じ DB / store に書けるよう、SW 側は
 * `firebase-messaging-sw.js` に同じロジックを inline 実装している (importScripts
 * 経由の shared module にすると build 側の設定が要るため duplicate で運用)。
 */

const DB_NAME = 'app-debug-log';
