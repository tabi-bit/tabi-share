...
const upsert = (trips: Trip[] | undefined, trip: Trip): Trip[] | undefined =>
...
export const writeTrip = (mutate: ScopedMutator, trip: Trip): void => {
  ...
  mutate(MY_TRIPS_KEY, (trips?: Trip[]) => upsert(trips, trip), { revalidate: false });
  mutate(MY_ARCHIVED_TRIPS_KEY, (trips?: Trip[]) => upsert(trips, trip), { revalidate: false });
};
